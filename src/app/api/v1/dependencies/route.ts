...
import { createDependency, isAdminRole, listDependencies } from "@/modules/identity";
...
export async function GET() {
  try {
    ...
    if (!isAdminRole(user)) requirePermission(user, "dependencies.read");
...
export async function POST(req: NextRequest) {
  try {
    const user = await getSession();
    if (!user) throw new AppError("No autenticado", 401);
    if (!isAdminRole(user)) requirePermission(user, "dependencies.create");
